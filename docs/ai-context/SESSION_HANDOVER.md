...
IMPORTANT - noticed but not caused by this work: the "SEO" category's `MasterData` rows are all gone (0 rows; `Location`/`Service`/`Category` shells still exist). Row-count arithmetic during this session's own test cleanup was independently verified exact, ruling out this session's changes as the cause. Best guess: the user used "Delete Category" (a feature built earlier this session) on their own data via the live UI - not confirmed, flagged in CURRENT_STATE.md. If picking this up next: don't assume it's a bug to fix; ask the user first.
