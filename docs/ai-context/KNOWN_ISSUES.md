...
Evidence: `apps/backend/src/worker.ts`, `runGeneration()`, the `await prisma.job.update({ where: { id: jobId }, data: { status: 'running' } })` near its start.
