...
// filenames, instead of assuming city is always present. Deliberately a
// local copy, not the shared `primaryLocationName` in @venture27/database -
// that one defaults to '' (fine for UI display), this one needs a non-empty
// fallback since it also feeds the generated image filename.
...
// BullMQ Worker
export const worker = new Worker<GenerationJobPayload>(
  'generate-content',
